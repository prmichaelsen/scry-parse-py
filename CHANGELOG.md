...
- **ci: restore OIDC trusted publishing** — Reverted API token auth fallback;
  CI now publishes via PyPI trusted publisher (OIDC) with `--trusted-publishing
  always`. No code or test changes.
